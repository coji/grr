...
**概要:** ユーザーが日記に未来のイベントを書いた時、翌日以降に「どうだった？」とフォローアップする機能。OpenClaw の HEARTBEAT 機構にインスパイアされた、プロアクティブな介入。
...
**HEARTBEAT の動作（cron との違い）:**

- **cron**: 盲目的にタスクを実行する（時間になったら必ず実行）
- **HEARTBEAT**: 「今それが意味があるか」を考えて判断する

1. 3時間ごとに「起きる」（9:00-21:00 JST のみ）
2. 各フォローアップを評価:
   - ユーザーは最近（3日以内）アクティブ？ → 非アクティブならスキップ
   - 同じユーザーに最近（24時間以内）送った？ → 連続送信を防止
   - イベント日は過ぎた？ → まだなら待機
3. 条件が揃った時だけ送信
4. 条件が揃わなければ静かにスリープ
...
- HEARTBEAT 設定: 活動時間 9-21 JST、ユーザー活動ウィンドウ 3日、最小間隔 24時間
