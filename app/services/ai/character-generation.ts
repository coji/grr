...
// ============================================
// Creative Seeds for Variety
// ============================================

/**
 * キャラクター生成時のバリエーションを増やすための「創造の種」
 */
 ...
 * ランダムな創造の種を生成
 */
function generateCreativeSeeds(): {
  theme: string
  personality: string
  keywords: string[]
} {
  const theme =
    CREATIVE_THEMES[Math.floor(Math.random() * CREATIVE_THEMES.length)]
  const personality =
  ...
  const keywordSet =
    INSPIRATION_KEYWORDS[
      Math.floor(Math.random() * INSPIRATION_KEYWORDS.length)
    ]

  return { theme, personality, keywords: [...keywordSet] }
}
...
function formatMemoriesForGeneration(memories: UserMemory[]): string {
  if (memories.length === 0) {
    // 記憶がない場合、ランダムな創造の種を使ってバリエーションを出す
    const seeds = generateCreativeSeeds()
    const season = getCurrentSeason()

    return `## 創造のヒント
まだこの人のことはわからないけど、以下のインスピレーションからユニークなキャラを創造してください。

### テーマ
${seeds.theme}をモチーフにしたキャラクター
...
### 季節
今は${season}。季節感を少し取り入れてもいい

### 重要
- 上記はあくまでヒント。自由に組み合わせて独創的に
- 「よくあるかわいいキャラ」ではなく、個性的で印象に残るキャラを
- 名前は響きが良く、覚えやすいものを`
  }
